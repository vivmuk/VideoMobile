...
- **Four modes, clearly labelled:** `IMAGE → VIDEO` (the default, because it is what most people want), `TEXT → VIDEO`, `REFERENCE → VIDEO`, and `VIDEO → VIDEO`. The mode filters the model list, changes the upload requirements, and renames the source panel. Transition models sit under `IMAGE → VIDEO`; motion-control and upscale models under `VIDEO → VIDEO`.
- **Every live Venice video model is selectable, newest first.** Each category opens with a `RECOMMENDED` shortlist of two or three models, then `ALL MODELS · NEWEST FIRST`. The shortlist is picked by model family — the newest variant of each preferred family — and is topped up with the newest models the category actually has, so a category made entirely of families the studio has never heard of still gets a shortlist. The most recent model in each category is labelled `NEW`.
- **Recency comes from Venice, not from a hand-written list.** Ordering keys on the `created` timestamp Venice stamps on each model, falling back to the generation parsed out of the model id (`wan-2-7` over `wan-2-5`, `seedance-2-0` over `seedance-1-0`, `ltx-2-v2-3`, `kling-o3`) and then the catalog's own order. Nothing in the picker needs editing when Venice ships another model.
- **Only supported parameters are shown, and nothing is invented.** Length, shape, quality, upscale factor, sound, image slots, and the prompt character limit come from that model's current `model_spec.constraints`. A list Venice does not publish stays empty, which means the control reads `Model decides` and the parameter is never sent — a guessed `5s` is worse than letting the model choose its own.
...
- **Prompt optimizer.** The wand button in the corner of the prompt box rewrites a rough draft into a full shot description — subject, action, setting, camera, light, atmosphere, style, motion detail, end state — through Venice's own text model, adapted to the selected video model, mode, clip length, framing and sound support. Pressing it again restores the creator's own wording, and typing clears the undo buffer. Set `VENICE_PROMPT_MODEL` to pin a specific text model; otherwise the `default` text trait is resolved from Venice and cached.
- **Model-family adapters.** The optimizer routes on the selected model's Venice id — not its display name — to a prompt adapter drawn from the `video-movie-prompting` skill in `.claude/skills/`. Grok Imagine, Seedance, Kling, Wan and Gemini Omni each get the ordering and emphasis their provider documents; every other model falls through to a neutral adapter rather than borrowing another provider's syntax. HappyHorse is deliberately evidence-gated: no provider prompt guide has been verified for it, so its drafts are rewritten conservatively and close to the creator's own wording. Family notes shape the paragraph only — the mode, sound, framing and character budget derived from the live `model_spec.constraints` always win.
...
- **Multiple images, only where the model accepts them.** The server declares how many images each model takes — one for ordinary image-to-video, two for transition models, and for reference models whatever the spec publishes, capped at the scheme's own limit (four for Kling, five otherwise). A thumbnail tray with add and remove appears only for models with room for more than one, so no slot is ever offered that Venice would reject. Transition models get `image_url` and `end_image_url`.
- **`@image` reference tags.** In `REFERENCE → VIDEO` every upload is addressable and named `@image1`, `@image2`, … The console shows the tag on each thumbnail and offers a row of chips that drop it into the description at the cursor, so a creator can write `@image1 hands a cup to @image2 at dusk` without learning any vendor syntax. On submission the server rewrites those tags into what the selected model actually reads — `@ElementN` with one `elements[]` entry per image for Kling, `@imageN` with a flat `reference_image_urls` array elsewhere — and loose spellings (`@Image 1`, `@ref2`, `@element3`) all canonicalise to the same tag. An untagged description still binds every attached reference rather than silently ignoring the extras.
- **The tags stay correct as you edit.** Removing a reference image renumbers the ones after it and rewrites the description's own tags to match, so a tag never quietly points at the wrong subject. A tag with no image behind it blocks `GENERATE` with the reason, and is refused server-side before Venice is called. The prompt optimizer is given the tag list and told to carry every one through a rewrite untouched.
...
- The advanced picker includes text-to-video, image-to-video, reference-image, two-image transition, video-to-video, motion-control, and video-upscale models. Every model in the account's catalog is placed in exactly one of the four categories, so a model is never hidden because its id is unfamiliar. It changes the required upload guidance for each model.
- Duration, aspect ratio, resolution, configurable audio, prompt limit, image slots, and upscale factor are derived from that model's current `model_spec.constraints`. A parameter that the model does not publish is not shown and not sent to Venice.
- Set `VENICE_BASE_URL` to point the studio at a Venice-compatible stub for local testing. Production leaves it unset.
...
- Reference tags read as plain English on screen (`@image1`) rather than as the provider token they become on the wire, so nobody has to know that Kling calls them elements.
